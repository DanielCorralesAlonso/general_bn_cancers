...
def pointwise_risk_mapping(model_bn, var1, var2, target = "CRC" ):
    ...
    A = model_infer.query(variables = [target], evidence = {})
...
def heatmap_plot_and_save(df, model_bn, target, col_var, row_var, q_length = 40, n_samples = 25000, path_to_data = "interval_df", interval = False, save = True):
    ...
    if interval:
        ...
        df_hom, df_muj = pointwise_risk_mapping(model_bn, col_var, row_var, target = target)
    ...
    if save:
        ...
        magnitude_data_hom.to_csv(f"riskmap_datasets/{target}/men_magnitudes_est_risk_map_{col_var}_{row_var}.csv")
        magnitude_data_muj.to_csv(f"riskmap_datasets/{target}/women_magnitudes_est_risk_map_{col_var}_{row_var}.csv")
